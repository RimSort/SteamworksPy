...
class SteamWorkshop(object):
    ...
    def SetItemSubscribedCallback(self, callback: object) -> bool:
        ...
        :return: bool
        """
        self._RemoteStorageSubscribePublishedFileResult = (
            self._RemoteStorageSubscribePublishedFileResult_t(callback)
        )
        self.steam.Workshop_SetItemSubscribedCallback(
            self._RemoteStorageSubscribePublishedFileResult
        )
        return True
    ...
    def SetItemUnsubscribedCallback(self, callback: object) -> bool:
        ...
        self._RemoteStorageUnsubscribePublishedFileResult = (
            self._RemoteStorageUnsubscribePublishedFileResult_t(callback)
        )
        self.steam.Workshop_SetItemUnsubscribedCallback(
            self._RemoteStorageUnsubscribePublishedFileResult
        )
        return True
    ...
    def SubscribeItem(
        ...
        if self._RemoteStorageSubscribePublishedFileResult is None:
            raise SetupRequired(
                "Call `SetItemSubscribedCallback` first or supply a `callback`"
            )
    ...
    def UnsubscribeItem(
        ...
        if self._RemoteStorageUnsubscribePublishedFileResult is None:
            raise SetupRequired(
                "Call `SetItemUnsubscribedCallback` first or supply a `callback`"
            )
    ...
    def SetItemDescription(self, update_handle: int, description: str) -> bool:
        ...
        return self.steam.Workshop_SetItemDescription(
            update_handle, description.encode()
        )
    ...
    def SetItemTags(self, update_handle: int, tags: list) -> bool:
        ...
        return self.steam.Workshop_SetItemTags(
            update_handle, pointer_storage, len(tags)
        )
    ...
    def SetItemContent(self, update_handle: int, content_directory: str) -> bool:
        ...
        :return: bool
        """
        return self.steam.Workshop_SetItemContent(
            update_handle, content_directory.encode()
        )
    ...
    def GetItemUpdateProgress(self, update_handle: int) -> dict:
        ...
        update_status = self.steam.Workshop_GetItemUpdateProgress(
            update_handle, pointer(punBytesProcessed), pointer(punBytesTotal)
        )
    ...
    def GetSubscribedItems(self, max_items: int = 0) -> list:
        ...
        actual_item_count = self.steam.Workshop_GetSubscribedItems(
            published_files, max_items
        )
    ...
    def GetItemInstallInfo(
        self, published_file_id: int, max_path_length: int = 1024
    ) -> dict:
    ...
    def GetItemDownloadInfo(self, published_file_id: int) -> dict:
        ...
        available = self.steam.Workshop_GetItemDownloadInfo(
            published_file_id, punBytesDownloaded, punBytesTotal
        )
        if available:
